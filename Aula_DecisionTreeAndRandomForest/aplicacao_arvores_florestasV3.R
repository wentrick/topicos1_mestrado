...
# Aplicacao: classificacao das especies de pinguins (Palmer)
...
# -------------------------------------------------------------
g_eda <- ggplot(dados, aes(x = bill_length_mm, y = flipper_length_mm,
                           color = species)) +
  geom_point(size = 2, alpha = 0.8) +
  labs(x = "Comprimento do bico (mm)", y = "Comprimento da nadadeira (mm)",
       color = "Especie", title = "Pinguins de Palmer: duas medidas ja separam bem") +
  theme_minimal(base_size = 13)

print(g_eda)
ggsave("figs/eda.png", g_eda, width = 7, height = 4.5, dpi = 150)
...
pdf("figs/arvore.pdf", width = 8, height = 5)
rpart.plot(arvore, type = 2, extra = 104, box.palette = "Blues")
dev.off()
...
print(g_comp)
ggsave("figs/comparacao.png", g_comp, width = 7, height = 4.5, dpi = 150)
...
print(g_imp)
ggsave("figs/importancia.png", g_imp, width = 7, height = 4.5, dpi = 150)
...
floresta_2d <- ranger(species ~ bill_length_mm + flipper_length_mm,
                      data = treino, num.trees = 500, seed = 2026)

grade <- expand_grid(
  bill_length_mm    = seq(min(dados$bill_length_mm), max(dados$bill_length_mm), length.out = 200),
  flipper_length_mm = seq(min(dados$flipper_length_mm), max(dados$flipper_length_mm), length.out = 200)
)
...
g_front <- ggplot(fronteiras, aes(bill_length_mm, flipper_length_mm)) +
...
print(g_front)
ggsave("figs/fronteiras.png", g_front, width = 9, height = 4.5, dpi = 150)